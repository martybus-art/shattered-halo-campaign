...
{toasts.map((t) => (
<div key={t.id} className={`pointer-events-auto rounded border px-4 py-3 shadow-2xl shadow-black/60 backdrop-blur-sm ${t.type === "success" ? "bg-void border-brass/60" : ""} ${t.type === "error"   ? "bg-void border-blood/60" : ""} ${t.type === "info"    ? "bg-void border-brass/30" : ""} `}>
<div className="flex items-start justify-between gap-3">
<div>
<p className={`text-sm font-semibold uppercase tracking-widest ${t.type === "success" ? "text-brass"    : ""} ${t.type === "error"   ? "text-blood"    : ""} ${t.type === "info"    ? "text-brass/70" : ""} `}>
{t.type === “success” && “* “}
{t.type === “error”   && “X “}
{t.type === “info”    && “- “}
{t.title}
...
className={`relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full border transition-colors duration-200 focus:outline-none disabled:opacity-40 ${ checked ? "bg-brass/50 border-brass/70" : "bg-void border-brass/30" }`}
>
<span className={`inline-block h-4 w-4 transform rounded-full bg-brass transition-transform duration-200 shadow-sm ${ checked ? "translate-x-6" : "translate-x-1" }`} />
</button>
{labelRight && (
<span className={`text-sm select-none ${checked ? "text-parchment/80" : "text-parchment/40"}`}>
{labelRight}
